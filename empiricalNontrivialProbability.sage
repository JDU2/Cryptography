...
def special_range_empirical_nontrivial_prob(bits, num_N=100, trials_per_N=10**5):
    """
    Generates 'num_N' random distinct semiprimes, with primes up to 'bits' bits. For each N, performs 'trials_per_N'
    trials to find x,y such that x^2 ≡ y^2 (mod N), for random unique x in range [floor(sqrt(n))+1, n-floor(sqrt(n))-1].
    Returns the average probability for x ≠ ±y (mod N), and total congruences of squares tested.
    """
